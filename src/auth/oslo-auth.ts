...
import { sessionTable } from "../db/sessions";
...
export function setSessionTokenCookie(
  ...
  if (env(c).ENVIRONMENT === "production") {
    c.res.headers.set(
      "Set-Cookie",
      `session=${token}; HttpOnly; SameSite=Lax; Max-Age=${expiresAt}; Path=/; Secure;`
    );
  } else {
    c.res.headers.set(
      "Set-Cookie",
      `session=${token}; HttpOnly; SameSite=Lax; Max-Age=${expiresAt}; Path=/`
    );
  }
}
